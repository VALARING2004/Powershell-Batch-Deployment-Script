...
├── deploy_parallel.ps1             # 主脚本（741行，核心引擎）
├── windows_update_control.ps1     # WinUpdate 控制模块（独立可复用）
├── windows_update_control.bat     # WinUpdate 控制批处理版本
├── uninstall_onedrive.bat          # OneDrive 七步卸载
├── unistall_cortana.bat            # Cortana 四管齐下禁用
├── windows10_taskbar_settings.bat  # 任务栏/开始菜单设置
├── hide_taskview.bat               # 隐藏任务视图按钮
├── progress_preview.bat            # 进度预览组件
├── deploy_7zip_2409_fixed.bat      # 7-Zip 单独安装（含修复）
├── deploy_all.bat                  # 全量部署入口（旧版串行）
├── deploy.bat                      # 基础部署入口（旧版）
├── test_wps_install.bat            # WPS 安装测试脚本
├── 常用软件 -2619.bat               # 26合1软件包索引
└── .gitignore
```
